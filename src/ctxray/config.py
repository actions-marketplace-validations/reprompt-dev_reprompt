...
def _default_db_path() -> str:
    ...
    return str(base / "ctxray" / "ctxray.db")
...
def _default_config_path() -> Path:
    ...
    return base.expanduser() / "ctxray" / "config.toml"


def _load_toml_config() -> dict[str, Any]:
    """Load TOML config file. Returns empty dict if file missing or invalid."""
    config_path_str = os.environ.get("CTXRAY_CONFIG_PATH")
    if config_path_str:
...
class Settings(BaseSettings):
    model_config = {"env_prefix": "CTXRAY_", "extra": "ignore"}
