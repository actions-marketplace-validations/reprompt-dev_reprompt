...
def template_save(
    ...
    from ctxray.core.templates import save_template
    from ctxray.storage.db import PromptDB
...
def template_use(
    ...
    from ctxray.config import Settings
    from ctxray.core.templates import extract_variables, render_template
    from ctxray.storage.db import PromptDB
    ...
    if template is None:
        ...
        console.print("Run [bold]ctxray template list[/bold] to see available templates.")
        raise typer.Exit(1)
...
def _load_plugins() -> None:
    """Auto-discover and load ctxray plugins (e.g. ctxray-pro)."""
    import logging
    ...
    try:
        ...
        eps = entry_points(group="ctxray.plugins")
        for ep in eps:
            try:
                register_fn = ep.load()
                register_fn(app)
...
def _version_callback(value: bool) -> None:
    if value:
        typer.echo(f"ctxray {__version__}")
        raise typer.Exit()
...
def scan(
    ...
    Examples:
        ...
        ctxray scan                                    # scan all AI tools

        ctxray scan --source cursor                    # scan Cursor only

        ctxray scan --source chatgpt-export --path ~/Downloads/conversations.json
    """
    from ctxray.config import Settings
    ...
    if stats.get("total_prompts", 0) > 0:
        ...
        if not hook_installed and db.get_setting("hook_suggestion_shown") is None:
            console.print(
                "\n[dim]Tip: auto-run ctxray after each session "
                "\u2192  ctxray install-hook claude-code[/dim]"
            )
...
def _hook_registered() -> bool:
    """Check if ctxray hook is registered in Claude Code settings."""
    import json
    ...
    try:
        ...
        return any(isinstance(h, dict) and "ctxray" in h.get("command", "") for h in hooks)
...
def import_file(
    ...
    for p in unique:
        try:
            ...
            logging.getLogger("ctxray.cli").debug(
...
def report(
    ...
    if total == 0:
        console.print("No prompts found. Run [bold]ctxray scan[/bold] to analyze your AI sessions.")
        return
    ...
    if html:
        ...
        out_path = Path(output) if output else Path("ctxray-report.html")
...
def library(
    ...
    """Deprecated: use `ctxray template list` instead."""
    console.print("[dim]library is now part of template list.[/dim]")
    console.print("[dim]Run: ctxray template list[/dim]")
    raise typer.Exit(0)
...
def trends(
    ...
    """Deprecated: use `ctxray digest --trends` instead."""
    console.print("[dim]trends is now part of digest --trends.[/dim]")
    console.print("[dim]Run: ctxray digest --trends[/dim]")
    raise typer.Exit(0)
...
def effectiveness(
    ...
    """Deprecated: use `ctxray insights` instead."""
    typer.echo(
        "Warning: 'effectiveness' is deprecated. Use 'ctxray insights' instead.",
        err=True,
    )
...
def status() -> None:
    ...
    console.print("[bold]ctxray status[/bold]")
...
def mcp_serve() -> None:
    ...
    try:
        from ctxray.mcp import run_server
    except ImportError:
        console.print(
            "[red]MCP support requires fastmcp.[/red]\n"
            "Install with: [bold]pip install ctxray\\[mcp][/bold]"
        )
        raise typer.Exit(1)
    run_server()
...
def purge(
    ...
    if all_:
        ...
        console.print("Run [bold]ctxray scan[/bold] to re-import your sessions.")
        return
...
def recommend(
    ...
    """Deprecated: use `ctxray template list --smart` instead."""
    console.print("[dim]recommend is now part of template list --smart.[/dim]")
    console.print("[dim]Run: ctxray template list --smart[/dim]")
    raise typer.Exit(0)
...
def merge_view(
    ...
    """Deprecated: use `ctxray insights` instead."""
    typer.echo(
        "Warning: 'merge-view' is deprecated. Use 'ctxray insights' instead.",
        err=True,
    )
...
def save(
    ...
    """Deprecated: use `ctxray template save` instead."""
...
def style(
    ...
    if trends:
        ...
        from ctxray.output.terminal import render_style_trends
...
def use(
    ...
    """Deprecated: use `ctxray template use` instead."""
...
def lint(
    ...
    Examples:
        ...
        ctxray lint --model claude                     # with Claude-specific hints
    ...
    from ctxray.config import Settings
    from ctxray.core.lint import format_lint_results, lint_prompts, load_lint_config
    from ctxray.core.pipeline import get_adapters
    from ctxray.storage.db import PromptDB

    settings = Settings()
    db = PromptDB(settings.db_path)

    # Load lint config from .ctxray.toml / pyproject.toml
    ...
    if not texts:
        console.print("No prompts found. Run [bold]ctxray scan[/bold] first.")
        raise typer.Exit(0)
...
def check(
    ...
    if not json_output:
        from ctxray.config import Settings as _S
        from ctxray.storage.db import PromptDB as _DB
...
def explain(
    ...
    Examples:
        ...
        ctxray explain "fix the auth bug"

        ctxray explain --file prompt.txt --json
    """
    text = _resolve_text(text, file)
...
def score(
    ...
    if json_output:
        ...
        from ctxray.output.terminal import render_score
...
def rewrite(
    ...
    Examples:
        ...
        ctxray rewrite "I was wondering if you could fix the authentication bug"

        ctxray rewrite --file prompt.txt --diff

        ctxray rewrite "please help me refactor this code to be better" --copy
    """
    text = _resolve_text(text, file)
    ...
    if json_output:
        ...
        from ctxray.output.rewrite_terminal import render_rewrite_diff

        typer.echo(render_rewrite_diff(result))
    else:
        from ctxray.output.rewrite_terminal import render_rewrite
...
def build(
    ...
    Examples:
        ...
        ctxray build "fix the auth bug"

        ctxray build "fix the crash" --file src/auth.ts --error "TypeError: ..."

        ctxray build "refactor" -f src/app.py --constraint "keep tests" --model claude
    """
...
def distill(
    ...
    Examples:
        ...
        ctxray distill                                 # distill most recent session

        ctxray distill --last 3 --summary              # summarize last 3 sessions

        ctxray distill --export --full                 # export context for a new session
    """
    ...
    if not sessions:
        if json_output:
            ...
            typer.echo("No sessions found. Run `ctxray scan` first to import sessions.")
        return
    ...
    if json_output:
        ...
        from ctxray.output.distill_terminal import render_distill, render_distill_summary
...
def compare(
    ...
    Examples:
        ...
        ctxray compare "fix bug" "Fix the login timeout in auth.py by adding retry logic"

        ctxray compare --best-worst                    # compare your best vs worst prompts

        ctxray compare --best-worst --source cursor
    """
    ...
    if best_worst:
        ...
        if pair is None:
            console.print(
                "Not enough scored prompts. Run [bold]ctxray scan[/bold]"
    ...
    if json_output:
        ...
        from ctxray.output.terminal import render_compare
...
def insights(
    ...
    if json_output:
        ...
        from ctxray.output.terminal import (
        ...
        if eff_data:
            ...
            console.print('  [dim]\u2192 Try: ctxray score "prompt" (improve weak patterns)[/dim]')

        if sim_data:
            typer.echo(render_similar_prompts_section(sim_data))
            console.print(
                '  [dim]\u2192 Try: ctxray template save "..." (reuse instead of rewrite)[/dim]'
            )
        ...
        if rep_data:
            ...
            console.print("  [dim]\u2192 Try: ctxray repetition (full analysis)[/dim]")
...
def privacy(
    ...
    Examples:
        ...
        ctxray privacy                                 # show what you sent where

        ctxray privacy --deep                          # scan for API keys, tokens, PII

        ctxray privacy --deep --json
    """
    import json as json_mod
    ...
    if deep:
        ...
        if json_output:
            ...
            from ctxray.output.terminal import render_privacy_deep
...
def digest(
    ...
    if history:
        ...
        if format == "json":
            ...
            from ctxray.output.terminal import render_digest_history
    ...
    if stats.get("total_prompts", 0) == 0:
        if quiet:
            print("ctxray: no data yet — run ctxray scan")
        elif format == "json":
            print(json_mod.dumps({"error": "no data", "hint": "run ctxray scan"}))
        else:
            console.print(
                "No prompt data yet. Run [bold]ctxray scan[/bold] first to populate the database."
            )
        return
...
def demo() -> None:
    """Run ctxray on demo data to see what it looks like."""
    import shutil
    import tempfile
    ...
    tmp = Path(tempfile.mkdtemp(prefix="ctxray-demo-"))
...
def install_hook(
    ...
    if source == "claude-code":
        ...
        hook_command = "ctxray scan --source claude-code"
        ...
        if with_digest:
            digest_command = "ctxray digest --quiet"
        ...
        console.print("ctxray will automatically scan when Claude Code sessions end.")
    else:
...
def _create_host_wrapper() -> Path:
    ...
    wrapper_dir = Path.home() / ".config" / "ctxray"
    wrapper_dir.mkdir(parents=True, exist_ok=True)
    wrapper_path = wrapper_dir / "ctxray-bridge-host"

    # Find the Python executable that has ctxray installed
    python_path = sys_mod.executable

    wrapper_path.write_text(f'#!/bin/sh\nexec "{python_path}" -u -m ctxray.bridge.host\n')
...
def extension_status() -> None:
    ...
    if registered_browsers:
        ...
        console.print("[yellow]Not registered.[/yellow] Run [bold]ctxray install-extension[/bold].")
    ...
    # Check last sync
    from ctxray.bridge.handler import _get_last_sync
...
def init(
    ...
    """Generate a .ctxray.toml config file in the current directory.

    Creates a starter config with all lint rules documented and commented.
    Use this to customize ctxray for your project or CI pipeline.

    Examples:

        ctxray init                     # create .ctxray.toml

        ctxray init --force             # overwrite existing config
    """
    config_path = Path.cwd() / ".ctxray.toml"
    ...
    config_content = """\
# ctxray configuration
# Docs: https://github.com/ctxray/ctxray
#
# This file configures `ctxray lint` rules and CI thresholds.
# Place in your project root — ctxray walks up from CWD to find it.
# Alternatively, add [tool.ctxray.lint] to pyproject.toml.

[lint]
# Fail `ctxray lint` if average prompt score < threshold (0 = disabled)
# Useful for CI: ctxray lint --score-threshold reads this value
# score-threshold = 50
...
"""
    ...
    console.print("  Edit rules, then run [bold]ctxray lint[/bold] to verify.")
...
def feedback() -> None:
    ...
    from ctxray.core.suggestions import FEEDBACK_URL
...
def projects(
    ...
    Examples:
        ...
        ctxray projects                          # all projects

        ctxray projects --source claude-code      # filter by source

        ctxray projects --json                   # machine-readable
    """
    import json as json_mod
...
def patterns(
    ...
    from ctxray.output.patterns_terminal import render_patterns
    from ctxray.storage.db import PromptDB
...
def agent(
    ...
    if not sessions:
        if json_output:
            ...
            typer.echo(
                "No agent sessions found. Run [bold]ctxray scan[/bold] to import sessions first."
            )
        return
    ...
    if json_output:
        ...
        from ctxray.output.agent_terminal import render_loops_only

        typer.echo(render_loops_only(agg), nl=False)
    else:
        from ctxray.output.agent_terminal import render_agent_report
